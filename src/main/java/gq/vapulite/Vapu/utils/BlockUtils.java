...
public class BlockUtils {
    ...
    public static boolean isOnLiquid() {
        ...
        AxisAlignedBB par1AxisAlignedBB = mc.thePlayer.getCollisionBoundingBox().offset(0.0D, -0.01D, 0.0D).contract(0.001D, 0.001D, 0.001D);
    ...
    public static boolean isInLiquid() {
        Minecraft.getMinecraft();
        AxisAlignedBB par1AxisAlignedBB = mc.thePlayer.getCollisionBoundingBox().contract(0.001D, 0.001D, 0.001D);
        int var4 = MathHelper.floor_double(par1AxisAlignedBB.minX);
        int var5 = MathHelper.floor_double(par1AxisAlignedBB.maxX + 1.0D);
        int var6 = MathHelper.floor_double(par1AxisAlignedBB.minY);
        int var7 = MathHelper.floor_double(par1AxisAlignedBB.maxY + 1.0D);
    ...
    public static LinkedList<BlockPos> findBlocksNearEntity(EntityLivingBase entity, int blockId, int blockMeta, int distance) {
        ...
        for (int x = (int) mc.thePlayer.posX - distance; x <= (int) mc.thePlayer.posX + distance; ++x) {
            for (int z = (int) mc.thePlayer.posZ - distance; z <= (int) mc.thePlayer.posZ + distance; ++z) {
                int height = mc.theWorld.getHeight();
    ...
    public static boolean CanStep() {
        Minecraft.getMinecraft();
        AxisAlignedBB par1AxisAlignedBB = mc.thePlayer.getCollisionBoundingBox().contract(0.0D, 0.001D, 0.0D);
        int var6 = MathHelper.floor_double(par1AxisAlignedBB.minY);
        int var7 = MathHelper.floor_double(par1AxisAlignedBB.maxY + 1.0D);
    ...
    public static boolean isInsideBlock() {
        Minecraft.getMinecraft();
        int x = MathHelper.floor_double(mc.thePlayer.getCollisionBoundingBox().minX);

        while (true) {
            Minecraft.getMinecraft();
            if (x >= MathHelper.floor_double(mc.thePlayer.getCollisionBoundingBox().maxX) + 1) {
                return false;
            }

            Minecraft.getMinecraft();
            int y = MathHelper.floor_double(mc.thePlayer.getCollisionBoundingBox().minY);

            while (true) {
                Minecraft.getMinecraft();
                if (y >= MathHelper.floor_double(mc.thePlayer.getCollisionBoundingBox().maxY) + 1) {
                    ++x;
                    break;
                }

                Minecraft.getMinecraft();
                int z = MathHelper.floor_double(mc.thePlayer.getCollisionBoundingBox().minZ);

                while (true) {
                    Minecraft.getMinecraft();
                    if (z >= MathHelper.floor_double(mc.thePlayer.getCollisionBoundingBox().maxZ) + 1) {
                        ++y;
                        break;
                    }
                    ...
                    if (block != null && !(block instanceof BlockAir)) {
                        ...
                        if ((boundingBox = block.getCollisionBoundingBox(var10001, var10002, mc.theWorld.getBlockState(new BlockPos(x, y, z)))) != null) {
                            Minecraft.getMinecraft();
                            if (mc.thePlayer.getCollisionBoundingBox().intersectsWith(boundingBox)) {
                                return true;
                            }
                        }
                    }
