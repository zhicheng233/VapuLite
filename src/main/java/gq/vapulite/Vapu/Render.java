...
public class Render {
	...
	public static void printBoundTextureInfo(int texture) {
		...
		LogManager.getLogger().log(Level.INFO, "texture %d parameters: width=%d, height=%d, depth=%d, format=%08x", texture, w, h, depth, format);		
	}
	...
	*/
	...
	public static void drawTexturedRect(double x, double y, double w, double h, double u1, double v1, double u2, double v2) {
		try {
	        ...
	        worlderRenderer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
	        //tes.startDrawingQuads();
	        worlderRenderer.pos(x + w, y,     zDepth).tex(u2, v1).endVertex();
	        worlderRenderer.pos(x,     y,     zDepth).tex(u1, v1).endVertex();
	        worlderRenderer.pos(x,     y + h, zDepth).tex(u1, v2).endVertex();
	        worlderRenderer.pos(x + w, y + h, zDepth).tex(u2, v2).endVertex();
	        tes.draw();
...
//        tes.startDrawing(GL11.GL_TRIANGLE_FAN);
        ...
        worlderRenderer.pos(x + (length * Math.cos(angle)), y + (length * Math.sin(angle)), zDepth).endVertex();
        worlderRenderer.pos(x + (length * 0.5D * Math.cos(angle - arrowBackAngle)), y + (length * 0.5D * Math.sin(angle - arrowBackAngle)), zDepth).endVertex();
        worlderRenderer.pos(x + (length * 0.3D * Math.cos(angle + Math.PI)), y + (length * 0.3D * Math.sin(angle + Math.PI)), zDepth).endVertex();
        worlderRenderer.pos(x + (length * 0.5D * Math.cos(angle + arrowBackAngle)), y + (length * 0.5D * Math.sin(angle + arrowBackAngle)), zDepth).endVertex();
        tes.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
...
//        tes.startDrawing(GL11.GL_TRIANGLES);
        WorldRenderer worlderRenderer = tes.getWorldRenderer();
        worlderRenderer.begin(GL11.GL_TRIANGLES, DefaultVertexFormats.POSITION);
        worlderRenderer.pos(x1, y1, zDepth).endVertex();
        worlderRenderer.pos(x2, y2, zDepth).endVertex();
        worlderRenderer.pos(x3, y3, zDepth).endVertex();
        tes.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
	}
...
//        tes.startDrawingQuads();
        ...
        worlderRenderer.pos(x + w, y,     zDepth).endVertex();
        worlderRenderer.pos(x,     y,     zDepth).endVertex();
        worlderRenderer.pos(x,     y + h, zDepth).endVertex();
        worlderRenderer.pos(x + w, y + h, zDepth).endVertex();
        tes.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
	}
...
//        tes.startDrawing(GL11.GL_TRIANGLE_STRIP);
		WorldRenderer worlderRenderer = tes.getWorldRenderer();
        worlderRenderer.begin(GL11.GL_TRIANGLE_STRIP, DefaultVertexFormats.POSITION);
        // for some the circle is only drawn if theta is decreasing rather than ascending
        double end = Math.PI * 2.0;
        double incr = end / circleSteps;
        double r2 = r + width;
        for (double theta = -incr; theta < end; theta += incr) {
        	worlderRenderer.pos(x + (r * Math.cos(-theta)), y + (r * Math.sin(-theta)), zDepth).endVertex();
        	worlderRenderer.pos(x + (r2 * Math.cos(-theta)), y + (r2 * Math.sin(-theta)), zDepth).endVertex();
        }
        tes.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
